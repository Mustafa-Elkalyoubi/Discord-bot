...
class ExtendedClient extends Client {
  ...
  public collector: DataCollector;
  ...
  constructor(options: ClientOptions, ownerID: string, token: string, collector: DataCollector) {
    super(options);
    ...
    this.commandManager = new CommandManager(this, ownerID, collector);
  ...
  public log(location: string, message: string, color: string = Modifiers.DEFAULT) {
    console.log(
      `${Modifiers.MAGENTA}${location} [${DateTime.now().toFormat("HH:mm:ss")}] ${color}${message}${Modifiers.DEFAULT}`
    );
  }
  ...
  public async getOSRSItems() {
    ...
    const res = await axios.get<OSRSWikiData>(apiURL, config).catch((e: Error) => {
      if (axios.isAxiosError(e)) {
        ...
        else console.error("Failed to connect to wiki servers");
      }
    });
    if (!res) return false;
  ...
  public toggleAI() {
    ...
    if (!this.aiProcess) {
      try {
        ...
        if (!this.aiProcess.pid) throw new Error("no pid");
      } catch (e) {
        if (e) throw e;
      }
    } else {
