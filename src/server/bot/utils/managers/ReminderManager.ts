...
export default class ReminderManager {
  ...
  async #loadJobs() {
    ...
    if (!reminders) throw new Error("Failed to load reminders");
  ...
  async save(
    ...
    if (!interaction.channel) throw new Error("No idea how this error occurred");
    ...
    try {
      ...
      if (recurring) {
        interaction.reply({
          content: `\`RECURRING\` Reminder [${saveDetails._id}] set! Reminding you ${cronstrue.toString(
        ...
        interaction.reply({
          content: `Reminder \`${saveDetails._id}\` set! Reminding you <t:${Math.floor(details.toSeconds())}:R>`,
        });
      }
